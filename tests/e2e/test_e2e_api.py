...
class TestE2EAPI:
    ...
    def test_store_and_retrieve_basic(self, api_client, e2e_config, test_user_id):
        ...
        content_found = any("sci-fi" in result.get("content", "").lower() for result in results)
        assert content_found, f"Expected sci-fi content in results: {results}"
    
    def test_store_and_retrieve_multiple_conversations(self, api_client, e2e_config, sample_conversations):
        ...
        for query, expected_keyword in queries:
            ...
            assert content_found, f"Expected '{expected_keyword}' in results for query '{query}': {results}"
    ...
    def test_retrieve_pagination(self, api_client, e2e_config, test_user_id):
        ...
        for i in range(5):
            store_payload = {
                "user_id": test_user_id,
                "history": [
                    {"role": "user", "content": f"I love topic {i} and enjoy learning about it."}
                ]
            }
            response = api_client.post(
                f"{e2e_config.api_base_url}/v1/store",
        ...
        response = api_client.get(
            f"{e2e_config.api_base_url}/v1/retrieve",
            params={"query": "user preferences", "user_id": test_user_id, "limit": 2, "offset": 0}
        )
        assert response.status_code == 200
        data = response.json()
        ...
        response = api_client.get(
            f"{e2e_config.api_base_url}/v1/retrieve",
            params={"query": "user preferences", "user_id": test_user_id, "limit": 2, "offset": 2}
        )
        assert response.status_code == 200
        data = response.json()
        assert len(data["results"]) <= 2
    ...
    def test_memory_quality_and_normalization(self, api_client, e2e_config, test_user_id):
    ...
    def test_structured_retrieval(self, api_client, e2e_config, test_user_id):
        ...
        response = api_client.get(
            ...
            params={"query": "user tasks and projects", "user_id": test_user_id, "limit": 10}
        )
        assert response.status_code == 200
        data = response.json()
        assert "results" in data
