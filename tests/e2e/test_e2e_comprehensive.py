...
class TestE2EComprehensive:
    ...
    def test_store_all_conversations(self, api_client, e2e_config, test_data):
        ...
        for i, conversation in enumerate(conversations):
            ...
            assert response.status_code == 200, f"Failed to store conversation {i}: {response.text}"
            
            data = response.json()
            assert data["memories_created"] > 0, f"No memories created for conversation {i}"
            
            print(f"Conversation {i+1}/{len(conversations)}: {data['memories_created']} memories created")
    ...
    def test_retrieve_with_various_queries(self, api_client, e2e_config, test_data):
        ...
        for query in queries:
            ...
            assert response.status_code == 200, f"Failed to retrieve for query '{query}'"
    ...
    def test_memory_quality_evaluation(self, api_client, e2e_config, test_data):
        ...
        assert metrics["precision"] >= 0.3, f"Precision too low: {metrics['precision']:.3f}"
    ...
    def test_memory_normalization_quality(self, api_client, e2e_config, test_data):
        ...
        assert len(normalization_issues) <= len(results) * 0.2, f"Too many normalization issues: {len(normalization_issues)}"
    ...
    def test_structured_retrieval(self, api_client, e2e_config, test_data):
        ...
        response = api_client.get(
            ...
            params={"query": "user preferences and activities", "user_id": test_user_id, "limit": 20}
        )
        assert response.status_code == 200
    ...
    def test_pagination_consistency(self, api_client, e2e_config, test_data):
        ...
        page1_response = api_client.get(
            f"{e2e_config.api_base_url}/v1/retrieve",
            params={"query": "user information", "user_id": test_user_id, "limit": 5, "offset": 0}
        )
        assert page1_response.status_code == 200
        
        page2_response = api_client.get(
            f"{e2e_config.api_base_url}/v1/retrieve",
            params={"query": "user information", "user_id": test_user_id, "limit": 5, "offset": 5}
        )
    ...
    def test_performance_under_load(self, api_client, e2e_config, test_data):
        ...
        def make_request():
            try:
                response = api_client.get(
                    f"{e2e_config.api_base_url}/v1/retrieve",
                    params={"query": "user information", "user_id": test_user_id, "limit": 10}
                )
